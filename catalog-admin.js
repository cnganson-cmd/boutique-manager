...
async function adminCatalog(message=''){
  ...
  try{
    const [items,brands,refs]=await Promise.all([
      authApi('produits','produit_id,nom_produit,variante,marque_id,categorie_produit_id,type_produit_id,actif','&order=nom_produit.asc'),
    ...
    shell(`<div class="title"><small>ADMINISTRATION · CATALOGUE</small><h1>Produits</h1><p>${items.length} produit${items.length>1?'s':''} · ${refs.filter(x=>x.actif).length} référence${refs.filter(x=>x.actif).length>1?'s':''} active${refs.filter(x=>x.actif).length>1?'s':''}.</p>${message?`<p><strong>${esc(message)}</strong></p>`:''}</div><div class="admin-toolbar"><button class="primary" onclick="adminNewProduct()">＋ Nouveau produit</button><button onclick="adminCatalogSettings()">Marques et catégories</button><button onclick="adminChooseProductImport()">Importer</button><button onclick="adminExportProducts()">Exporter</button><input id="admin-product-import" type="file" accept=".csv,text/csv" hidden onchange="adminImportProducts(this.files[0])"></div><div class="admin-filters"><label for="admin-product-search">Rechercher<input id="admin-product-search" autocomplete="off" placeholder="Marque, produit, variante…" oninput="adminFilterProducts()"></label><label for="admin-product-status">Afficher<select id="admin-product-status" onchange="adminFilterProducts()"><option value="all">Tous</option><option value="active">Actifs</option><option value="inactive">Inactifs</option></select></label></div><section class="list" id="admin-product-list">${items.map(p=>{const productRefs=refs.filter(r=>r.produit_id===p.produit_id),search=`${brandNames[p.marque_id]||''} ${p.nom_produit} ${p.variante||''}`.toLowerCase();return `<article class="product-row admin-product-item" data-search="${esc(search)}" data-active="${Boolean(p.actif)}"><div class="grow"><small>${esc(brandNames[p.marque_id]||'Sans marque')} · ${p.actif?'ACTIF':'INACTIF'}</small><strong>${esc(p.nom_produit)}</strong><span>${esc(p.variante||'Sans variante')} · ${productRefs.length} référence${productRefs.length>1?'s':''}</span></div><button class="row-action" onclick="adminEditProduct('${p.produit_id}')">Modifier</button></article>`}).join('')||'<div class="empty">Aucun produit.</div>'}</section>${nav('products')}`,'adminConsole()');
