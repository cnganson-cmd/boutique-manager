...
async function submitLineDecision(fluxId,lineId,decision,qty){const comment=document.querySelector('#decisionComment')?.value?.trim()||null;if(decision==='REFUSE'&&!confirm('Confirmer le refus de cette ligne ?'))return;try{await request('/rest/v1/rpc/traiter_ligne_stock_idempotent',{method:'POST',auth:true,body:{p_operation_client_id:stockOpId('decision-'+lineId),p_ligne_flux_stock_id:lineId,p_decision:decision,p_quantite:qty,p_commentaire:comment}});clearStockOpId('decision-'+lineId);await requestDetail(fluxId)}catch(err){alert(err.message)}}
async function shipRequest(fluxId){
  ...
  try{
    await request('/rest/v1/rpc/expedier_flux_stock_idempotent',{method:'POST',auth:true,body:{p_operation_client_id:stockOpId('ship-'+fluxId),p_flux_stock_id:fluxId,p_commentaire:'Expédition confirmée par le magasinier depuis Boutique Manager'}});
