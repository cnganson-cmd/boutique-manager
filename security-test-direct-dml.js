async function testDirectDmlSecurity(){
 ...
 try{
  await request('/rest/v1/stocks_courants?limit=1',{method:'PATCH',auth:true,body:{quantite:987654321}});
  if(o)o.innerHTML='<p><strong>❌ ÉCHEC SÉCURITÉ :</strong> Supabase a accepté une modification directe de stocks_courants.</p>';
 }catch(e){
...
function directDmlSecurityScreen(){if(!currentUser)return false;shell(`<div class="title"><small>TEST DE SÉCURITÉ</small><h1>Écriture directe DML → refus</h1><p>La session authentifiée de ${esc(currentUser.nom)} tente de modifier directement la table stocks_courants, sans passer par une RPC métier.</p></div><div id="direct-dml-result" class="empty">Aucune écriture n'a encore été tentée.</div><button class="primary" onclick="testDirectDmlSecurity()">Tester l'écriture directe</button>`,'home()');return true;}
